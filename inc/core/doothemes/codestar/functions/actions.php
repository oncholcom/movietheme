        ...
        if( !empty($jsons) ){
            foreach ($jsons as $path) {
                ...
                if( is_object( $object ) ) {
                    ...
                    echo '<h4 class="cs-icon-title">'. esc_html__( 'Error! Can not load json file.', 'mtms' ) .'</h4>';
                }
            }
        }
...
if( !function_exists('cs_set_icons') ){
    function cs_set_icons() {
        $out = '<div id="cs-icon-dialog" class="cs-dialog" title="'. esc_html__( 'Add Icon', 'mtms' ) .'">';
        $out .= '<div class="cs-dialog-header cs-text-center"><input type="text" placeholder="'. esc_html__( 'Search a Icon...', 'mtms' ) .'" class="cs-icon-search" /></div>';
        $out .= '<div class="cs-dialog-load"><div class="cs-icon-loading">'. esc_html__( 'Loading...', 'mtms' ) .'</div></div>';
        $out .= '</div>';
        echo $out;
    }
