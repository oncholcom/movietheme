...
if( !function_exists('cs_add_element') ){
    function cs_add_element( $field = array(), $value = '', $unique = '' ) {
        ...
        if( class_exists( $class ) ) {
            ...
            $output .= '<p>'. esc_html__( 'This field class is not available!', 'mtms' ) .'</p>';
        }
...
if( ! function_exists( 'cs_encode_string' ) ) {
    function cs_encode_string($string, $code = '64') {
        return rtrim( strtr( call_user_func( 'base'. $code .'_encode', addslashes( gzcompress( serialize( $string ), 9 ) ) ), '+/', '-_' ), '=' );
    }
}
...
if( !function_exists('cs_decode_string') ){
    function cs_decode_string( $string, $code = '64') {
        return unserialize( gzuncompress( stripslashes( call_user_func( 'base'. $code .'_decode', rtrim( strtr( $string, '-_', '+/' ), '=' ) ) ) ) );
    }
}
